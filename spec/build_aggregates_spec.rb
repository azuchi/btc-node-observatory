...
RSpec.describe 'btc-node-data/scripts/build_aggregates.rb' do
  ...
  def snapshot(ts, network_class, inst, union)
    ...
    base = { 'ts' => ts, 'network_class' => network_class, 'candidates' => 1000,
             'instantaneous' => inst, 'union_24h' => union,
             'by_fail_reason' => { 'timeout' => 7, 'refused' => 3 }, 'by_network' => by_network,
  ...
  it 'builds per-granularity aggregates from daily files (clearnet/onion kept separate)' do
    Dir.mktmpdir do |tmp|
      ...
      expect(clearnet['by_fail_reason']).to eq({ 'timeout' => 7, 'refused' => 3 })
      expect(latest['networks']['onion']['by_network']).to eq({ 'onion' => 3002 })
      expect(latest['networks']['onion']).not_to have_key('by_country')
      expect(latest['networks']['onion']['by_fail_reason']).to eq({ 'timeout' => 7, 'refused' => 3 })
    end
  end

  it 'omits by_fail_reason for daily files predating it rather than emptying it' do
    # Every daily file before 2026-08-17 lacks the field. compact must drop the
    # key, so the dashboard can tell "not published then" from "no failures".
    Dir.mktmpdir do |tmp|
      repo = File.join(tmp, 'repo')
      FileUtils.mkdir_p(File.join(repo, 'scripts'))
      FileUtils.cp(script, File.join(repo, 'scripts'))

      old = snapshot(Time.utc(2026, 7, 20).to_i, 'clearnet', 5000, 5500)
      old.delete('by_fail_reason')
      write_daily(repo, '2026-07-20', [old])

      out, status = Open3.capture2e('ruby', File.join(repo, 'scripts', 'build_aggregates.rb'))
      expect(status).to be_success, out

      latest = JSON.parse(File.read(File.join(repo, 'aggregates', 'latest.json')))
      expect(latest['networks']['clearnet']).not_to have_key('by_fail_reason')
    end
  end
end
